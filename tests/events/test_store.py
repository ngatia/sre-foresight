...
async def test_record_and_query_window(store):
    ...
    rows = await store.recent_for_service(
        "resume", now - timedelta(minutes=5), now + timedelta(minutes=5)
    )
...
async def test_window_excludes_out_of_range(store):
    now = datetime.now(UTC)
    await store.record(
        ChangeEventInput("resume", "deploy", "argocd", "old", now - timedelta(hours=2))
    )
    rows = await store.recent_for_service(
        "resume", now - timedelta(minutes=5), now + timedelta(minutes=5)
    )
